...
def new(name, price, description, file):
    sql = text("""INSERT INTO products (name, price, description)
...
def review(product_id, username, rating, content):
    sql = text("""INSERT INTO reviews (product_id, username, rating, content, created_at)
               VALUES (:product_id, :username, :rating, :content, NOW())""")
    db.session.execute(sql, {"product_id":product_id, "username":username, \
                              "rating":rating, "content":content})
    db.session.commit()

def get_reviews(product_id):
    sql = text("""SELECT username, rating, content,
